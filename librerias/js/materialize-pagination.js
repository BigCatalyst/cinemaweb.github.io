...
(function ($, window, document, undefined) {
  ...
  MaterializePagination.prototype = {
    ...
    init: function () {
      ...
      var requestedPage = this.config.useUrlParameter
        ? this.parseUrl()
        : this.config.firstPage;
      // Create initial pagination and add it to the DOM
      if (this.createPaginationBase(requestedPage)) this.bindClickEvent();
    },

    createPaginationBase: function (requestedPage) {
      if (isNaN(this.config.firstPage) || isNaN(this.config.lastPage)) {
        console.error(
          "Both firstPage and lastPage attributes need to be integer values"
        );
        return false;
      } else if (this.config.firstPage > this.config.lastPage) {
        console.error(
          "Value of firstPage must be less than the value of lastPage"
        );
        return false;
      }
    ...
    requestPage: function (requestedPage, initing) {
      ...
      if (!initing) this.config.onClickCallback(this.currentPage);
      this.renderActivePage();
    ...
    requestPrevPage: function () {
      ...
      this.visiblePages.unshift(
        this.insertPrevPaginationComponent(this.currentPage - 1)
      );
    },
    ...
    requestNextPage: function () {
      ...
      this.visiblePages.push(
        this.insertNextPaginationComponent(this.currentPage + 1)
      );
    },
    ...
    requestPageByNumber: function (requestedPage) {
      ...
      this.visiblePages.push(
        this.insertNextPaginationComponent(this.currentPage)
      );
      this.visiblePages.push(
        this.insertNextPaginationComponent(this.currentPage + 1)
      );
    ...
    renderActivePage: function () {
      ...
      var currentPageComponent = $(
        this.$container.find('[data-page="' + this.currentPage + '"]')[0]
      );
    ...
    insertPrevPaginationComponent: function (pageNumber) {
      if (
        pageNumber > this.config.firstPage &&
        pageNumber < this.config.lastPage
      ) {
    ...
    parseUrl: function () {
      var requestedPage =
        this.getUrlParamByName(this.config.urlParameter) ||
        this.config.firstPage;
